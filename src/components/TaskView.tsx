...
const TaskView = ({ id, title, open, setOpen, setColor }: Props) => {
  ...
  const handlerComplete = () => {
    axios
      .put("https://tasks-app-api.herokuapp.com/tasks/" + id)
      .then((response) => {
        alert(response.data);
        setOpen(false);
        setColor("green-500");
          ...
          >
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button
                  type="button"
                  className="w-full mt-3 inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
